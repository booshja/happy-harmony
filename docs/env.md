...
- **Runtime (required for server features):**
    ...
    3. Apply local DB migrations: `pnpm db:migrate`
    4. Run `pnpm dev` (uses `wrangler dev` on port 3000)
...
- **Schema:** defined in `src/db/schema.ts` (Drizzle ORM); access via `getDb()` from `src/db/index.ts` (Workers runtime only).
- **Migrations:** SQL files generated by drizzle-kit, stored in `drizzle/`, applied via wrangler.

| Command                   | What it does                                                                                              |
| ------------------------- | --------------------------------------------------------------------------------------------------------- |
| `pnpm db:generate`        | Generate a new SQL migration file from schema changes                                                     |
| `pnpm db:migrate`         | Apply pending migrations to the local D1 (wrangler dev)                                                   |
| `pnpm db:migrate:staging` | Apply pending migrations to staging D1 (requires `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` in env) |
| `pnpm db:seed`            | Insert seed rows into the local D1 (from `drizzle/seeds/seed.sql`)                                        |
| `pnpm db:reset`           | Wipe local D1 state and reapply all migrations from scratch (local only)                                  |

- **PR previews** share the staging D1 database. This is intentional — separate per-PR databases are not provisioned.
- **Type generation:** run `pnpm cf-typegen` after changing `wrangler.jsonc` bindings to keep `worker-configuration.d.ts` up to date.
